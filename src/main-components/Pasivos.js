	...
	render(){
		...
		return(
			...
			<Grid item sm={3} className="sidebar">
				...
				<ul>
					<li>
						<Link className={ !section  ?"router-link-exact-active router-link-active" : ""} to={`${process.env.PUBLIC_URL}/perfil/${this.props.profile._id}/pasivos/`}>
							Resumen
						</Link>
					</li>
					<li>
		  ...
		  <Switch>
				<Route exact path={`${process.env.PUBLIC_URL}/perfil/:id/pasivos`} render={() => <PasivosResume profile={this.props.profile}  />}/>
		    <Route exact path={`${process.env.PUBLIC_URL}/perfil/:id/pasivos/deudas`} render={() => <PasivosDeudas profile={this.props.profile}  />}/>
		    <Route exact path={`${process.env.PUBLIC_URL}/perfil/:id/pasivos/otras-obligaciones`} render={() => <PasivosObligaciones profile={this.props.profile}  />}/>
		  </Switch>
	  </div>
		);
	}
